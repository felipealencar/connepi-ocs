...
{if $allowRegReader || $allowRegAuthor || $allowRegReviewer}
	<tr valign="top">
		...
		<td class="value">
			...
			{/if}
			{if $allowRegAuthor}
				<input type="checkbox" id="authorRole" name="authorRole" {if $isAuthor || $authorRole}checked="checked" {/if} readonly/>&nbsp;{fieldLabel name="authorRole" key="user.role.author"}<br/>
			{/if}
			{if $allowRegReviewer}
